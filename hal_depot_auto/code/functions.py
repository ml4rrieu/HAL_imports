...
def addRow(docId, state, treat_info='', hal_match='', uris=''):
	...
	if not docId['doctype'] : docId['doctype'] = 'not include'
	if treat_info : print(f"\t{treat_info}")
	if hal_match : print(f"\t{hal_match}")
	if uris : print(f"\t{','.join(uris)}")
...
def load_tables_and_create_outputs():
	...
	# load matching table : afid2halid
	global afid2structId
	with open(local_data["path_scopusStruct2halStruct"], 'r', newline='', encoding="utf-8") as table :
		reader = csv.DictReader(table)
		afid2structId = {row["afid"] : row["halid"] for row in reader}
	...
	global docs_table
	docs_table = open('../out/doc_analysed.csv', 'w', newline='', encoding="utf8")
	global writeDoc
	...
	global affil_txt
	affil_txt = io.open('../out/affil_analysed.txt', 'w', encoding='utf-8')


def loadLabCriteria():
	t = open(local_data["path_labCriteria"], newline='', encoding='utf8')
	reader = csv.DictReader(t)
	temp = {row['sigle'].strip() : row.copy() for row in reader}
	return temp

#______1_____ ENRICH & STRUCTURE AUTHORS DATA

def matchDocType(doctype):
	doctype_scopus2hal = {'Article': 'ART', 'Review':'ART', 'Business Article':'ART', "Data Paper":"ART", 'Conference Paper':'COMM',\
...
def finish(step): 	
	if step == "verifAuth" : 
		...
		with open('../out/temp_uvsq_authors.csv', 'w', newline='',encoding='utf8') as fh :
